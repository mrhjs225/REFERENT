...
from prepare_data import create_all_test_data
...
parser.add_argument("-kf", "--k-fold", type=int, default=0)
parser.add_argument("-ut", "--use-t5", type=boolean_string, default=False)
parser.add_argument("-ta", "--test-all", type=boolean_string, default=False)
parser.add_argument("-dd", "--dataset-dir", type=str, default="")
parser.add_argument("-bid", "--bug-id-num", type=int, default="")


args = parser.parse_args()

bug_id_num = args.bug_id_num
...
data = GetDataAsPython(dataset_dir)
...
if args.test_all:
    (
        ...
        val_info,
        test_info,
    ) = create_all_test_data(data, all_warning_types, include_warning=True, model_name=model_name)
else:
    k_fold = args.k_fold
    (
        train_inputs,
        ...
        val_info,
        test_info,
    ) = create_data(data, all_warning_types, include_warning=True, model_name=model_name, k_fold=k_fold)
...
for i, warning in enumerate(all_warning_types):
    ...
    correctness = np.all(np.equal(target_ids, output_ids), axis=1)
    i = 0
    for k, output_id in enumerate(output_ids):
        ...
        if correctness[i] == True:
            test_warning_info[k].correctness = 'True'
        else:
            test_warning_info[k].correctness = 'False'
        i += 1
